...
function buildTabFilterCSS( visibleTabs ) {
	const hiddenIndices = ALL_TABS
		.map( ( name, i ) => ( visibleTabs.includes( name ) ? null : i + 1 ) )
		.filter( Boolean );
	...
	for ( const nth of hiddenIndices ) {
		rules.push(
			`.iso-editor__inserter-tabs .block-editor-tabbed-sidebar__tab:nth-child(${ nth }) { display: none; }`,
			`.iso-editor__inserter-tabs .block-editor-tabbed-sidebar__tabpanel:nth-of-type(${ nth }) { display: none; }`
		);
	}
	...
	if ( visibleTabs.length === 1 ) {
		rules.push(
			`.iso-editor__inserter-tabs .block-editor-tabbed-sidebar__tablist-and-close-button { display: none; }`
		);
	}
 ...
 */
export default function InserterSidebar( { canClose = true, tabs } ) {
	...
	return (
		...
		>
			...
			<div className="edit-widgets-layout__inserter-panel-content iso-editor__inserter-tabs">
