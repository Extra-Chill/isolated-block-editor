...
async function coreMediaFetch(query = {}, mediaType) {
  ...
  let mediaItems;
  try {
    mediaItems = await coreSelect.getEntityRecords('postType', 'attachment', {
      ...query,
 ...
 */
export default function MediaCategoryRegistrar({
  settings
}) {
  const didRegister = useRef(false);
  const allowedMimeTypes = settings?.editor?.allowedMimeTypes;
  const hasMimeTypes = allowedMimeTypes && typeof allowedMimeTypes === 'object' && Object.keys(allowedMimeTypes).length > 0;
  const {
