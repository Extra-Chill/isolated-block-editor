...
### Fixed
- render Popover.Slot inside DetachedSidebar so preview popovers stack with their anchors

## [3.3.2] - 2026-04-27

### Changed
- Remove unsupported Homeboy build_command
- migrate HotSwapper from withSelect/withDispatch to hooks
- migrate PatternMonitor from withSelect/withDispatch to hooks

### Fixed
- type HotSwapper registry access
- reset HotSwapper target on unmount

## [3.3.1] - 2026-04-26

### Fixed
- thread editor.styles through to VisualEditor / BlockCanvas

## [3.3.0] - 2026-04-02

### Added
- fix detached sidebar scroll, add tab filtering config, auto-register media categories

### Changed
- Fire enqueue_block_assets in example PHP for Gutenberg 22.8+ iframe compatibility

## [3.2.1] - 2026-03-27

### Fixed
- constrain inserter panel content height in detached sidebar

## [3.2.0] - 2026-03-27

### Added
- add detached sidebar support for compose surfaces

### Changed
- update build artifacts for mobile detached sidebar fix

### Fixed
- prevent horizontal overflow on sub-340px viewports
- suppress header on mobile when persistent detached sidebar is active
- fix changelog: clean up mangled 3.1.2 entry

## [3.1.2] - 2026-03-24

### Added

-   `replaceContent(blocks)` store action — replaces all editor content and resets undo/redo history. Use when loading new documents, switching drafts, or restoring saved content. Takes parsed block objects.
-   `getSerializedContent()` store selector — returns the current editor content as serialized block markup (HTML).

### Changed

-   Registered as a homeboy component for proper versioning and release management.
